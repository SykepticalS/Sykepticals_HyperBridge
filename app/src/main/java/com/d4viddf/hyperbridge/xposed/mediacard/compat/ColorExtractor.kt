...
object ColorExtractor {
    ...
    private const val MAX_SAMPLE_PIXELS = 36 * 36
    private const val KMEANS_ITERATIONS = 6
    ...
    fun extract(bitmap: Bitmap, maxColors: Int = DEFAULT_MAX_COLORS): List<Int> {
        ...
        val scaled = scaleBitmap(bitmap, MAX_SAMPLE_PIXELS) ?: return emptyList()
        ...
        val k = (maxColors + 2).coerceIn(1, size)
    ...
    private fun scaleBitmap(bitmap: Bitmap, maxPixels: Int): Bitmap? {
        val totalPixels = bitmap.width.toLong() * bitmap.height
        val alreadySmall = bitmap.config != Bitmap.Config.HARDWARE && totalPixels <= maxPixels
        if (alreadySmall) return bitmap
        return MediaArtworkSampler.sample(bitmap, maxPixels)
    }
