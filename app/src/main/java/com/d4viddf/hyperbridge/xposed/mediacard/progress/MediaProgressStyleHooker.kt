...
package com.d4viddf.hyperbridge.xposed.mediacard.progress
...
object MediaProgressStyleHooker {
    ...
    fun hook(xposedModule: XposedModule, classLoader: ClassLoader) {
        ...
        if (
            notificationStyle == MediaCardConstants.NOTIFICATION_MEDIA_PROGRESS_STYLE_DEFAULT &&
            !notificationHeadGlow &&
            islandStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_PROGRESS_STYLE_DEFAULT
        ) {
            return
        }

        val needsWaveProgressApi =
            notificationStyle == MediaCardConstants.NOTIFICATION_MEDIA_PROGRESS_STYLE_WAVE ||
                islandStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_PROGRESS_STYLE_WAVE
        val commonApi = if (needsWaveProgressApi) {
            runCatching { CommonApi.create(classLoader) }
                .onFailure {
                    MediaCardLog.w(TAG, "媒体进度数据接口不可用: reason=${it.message}")
                }
                .getOrNull()
        } else {
            null
        ...
        if (islandStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_PROGRESS_STYLE_WAVE) {
            installed += installIslandHooks(xposedModule, classLoader, commonApi)
        }

        if (installed == 0) {
            hookedClassLoaders.remove(classLoader)
            MediaCardLog.w(TAG, "未找到兼容的媒体进度条 Hook")
        } else {
            MediaCardLog.d(
                TAG,
    ...
    private fun installNotificationHooks(
        ...
        if (
            style == MediaCardConstants.NOTIFICATION_MEDIA_PROGRESS_STYLE_WAVE &&
            commonApi == null
        ) {
            return 0
        }
        val api = runCatching {
            ...
            MediaCardLog.w(TAG, "通知中心进度条接口不可用: reason=${it.message}")
        }.getOrNull() ?: return 0
        ...
        if (
            ...
            MediaCardLog.w(TAG, "通知中心进度或拖动接口不可用，跳过波浪进度条替换")
            return 0
        }
        ...
        api.holderConstructors.forEach { constructor ->
            runCatching {
                ...
                MediaCardLog.e(TAG, "安装通知中心进度条构造 Hook 失败", it)
            }
        }
    ...
    private fun installIslandHooks(
        ...
        val api = runCatching {
            ...
            MediaCardLog.w(TAG, "超级岛进度条接口不可用: reason=${it.message}")
        }.getOrNull() ?: return 0
        ...
        api.holderConstructors.forEach { constructor ->
            runCatching {
                ...
                MediaCardLog.e(TAG, "安装超级岛进度条构造 Hook 失败", it)
            }
        }
    ...
    private inline fun installMethod(
        ...
        runCatching {
            ...
            MediaCardLog.e(TAG, "安装媒体进度条 Hook 失败: method=${method.name}", it)
        }
    }
    ...
    private class NotificationHolderHook(
        ...
        override fun intercept(chain: Chain): Any? {
            ...
            runCatching {
                ...
                MediaCardLog.e(TAG, "初始化通知中心进度条失败", it)
            }
            return result
        }
    }
        ...
        override fun intercept(chain: Chain): Any? {
            ...
            chain.thisObject?.let { holder ->
                runCatching { api.replaceWithWave(holder) }
                    .onFailure { MediaCardLog.e(TAG, "初始化超级岛波浪进度条失败", it) }
            }
            return result
        }
    }
    ...
    private class NotificationControllerHook(
        ...
        override fun intercept(chain: Chain): Any? {
            ...
            runCatching {
                ...
                MediaCardLog.e(TAG, "同步通知中心波浪进度条状态失败: action=$action", it)
            }
            return result
        }
    }
    ...
    private class IslandBinderHook(
        ...
        override fun intercept(chain: Chain): Any? {
            ...
            runCatching {
                ...
                MediaCardLog.e(TAG, "同步超级岛波浪进度条状态失败: action=$action", it)
            }
            return result
        }
    }
        ...
        fun replaceWithNativeGlow(holder: Any) {
            ...
            try {
                ...
                if (shaderField.get(replacement) == null) {
                    MediaCardLog.w(TAG, "当前设备级别不支持原生光辉进度条，保留通知中心默认样式")
                    return
                }
    ...
    private fun replaceWithWaveSeekBar(
        ...
        val replacement = SquigglySeekBar(context).apply {
            ...
            this.thumbStyle = when (thumbStyle) {
                MediaCardConstants.NOTIFICATION_MEDIA_THUMB_STYLE_VERTICAL -> ThumbStyle.VerticalBar
                MediaCardConstants.NOTIFICATION_MEDIA_THUMB_STYLE_HIDDEN -> ThumbStyle.Hidden
