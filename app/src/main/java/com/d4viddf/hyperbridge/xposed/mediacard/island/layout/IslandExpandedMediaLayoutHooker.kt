package com.d4viddf.hyperbridge.xposed.mediacard.island.layout
...
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.coloros.IslandExpandedMediaColorOsLayoutPreset
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.coloros.IslandExpandedMediaColorOsMetrics
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.ios.IslandExpandedMediaIosLayoutPreset
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.ios.IslandExpandedMediaIosMetrics
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.oneui.IslandExpandedMediaOneUiLayoutPreset
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.miui.IslandExpandedMediaMiuiLayoutPreset
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.pixel.IslandExpandedMediaPixelLayoutPreset
...
object IslandExpandedMediaLayoutHooker {
    ...
    fun hook(xposedModule: XposedModule, classLoader: ClassLoader) {
        ...
        val api = try {
            ...
            if (unavailableNativeApis.add(classLoader)) {
                MediaCardLog.w(TAG, "跳过超级岛展开态媒体布局 Hook: reason=${error.message}")
            }
            return
        }
        ...
        MediaCardLog.d(TAG, "超级岛展开态媒体布局 Hook 已初始化: methods=${handles.size}")
    }
    ...
    private class NativeApi private constructor(
        ...
        fun applyCurrentLayout(constraintSet: Any, context: Context) {
            ...
            runCatching {
                ...
                when (MediaCardRuntimeConfig.current.islandExpanded.layoutStyle) {
                    ...
                    MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_PIXEL ->
        ...
        fun applyElementOverrides(
            ...
            val ordered = when (config.actionOrder) {
                MediaCardConstants.ISLAND_EXPANDED_MEDIA_ACTION_ORDER_CUSTOM_RIGHT ->
            ...
            if (config.coverStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_COVER_STYLE_HIDDEN) {
            ...
            if (config.hideDeviceSwitch) {
                ...
                if (config.layoutStyle !=
                    MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_COLOROS &&
                    config.layoutStyle != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_ONEUI
                ) {
    ...
    private fun resolveApi(classLoader: ClassLoader?): NativeApi? {
        ...
        return try {
            ...
            if (unavailableNativeApis.add(classLoader)) {
                MediaCardLog.w(TAG, "超级岛展开态媒体布局原生接口不可用: reason=${error.message}")
            }
            null
        }
    }
