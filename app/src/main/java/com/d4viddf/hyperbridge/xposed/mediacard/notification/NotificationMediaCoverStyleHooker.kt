package com.d4viddf.hyperbridge.xposed.mediacard.notification
...
import com.d4viddf.hyperbridge.xposed.mediacard.MediaCoverRotationController
import com.d4viddf.hyperbridge.xposed.mediacard.notification.aod.NotificationMediaFullAodAnimatedHeightHook
import com.d4viddf.hyperbridge.xposed.mediacard.notification.aod.NotificationMediaFullAodHook
import com.d4viddf.hyperbridge.xposed.mediacard.notification.background.NotificationMediaBackgroundController
import com.d4viddf.hyperbridge.xposed.mediacard.notification.layout.NotificationMediaLayoutController
import com.d4viddf.hyperbridge.xposed.mediacard.notification.layout.NotificationMediaLayoutResourceIds
import com.d4viddf.hyperbridge.xposed.mediacard.notification.layout.miui.NotificationMediaMiuiStyle
import com.d4viddf.hyperbridge.xposed.mediacard.notification.layout.oneui.NotificationMediaOneUiStyle
import com.d4viddf.hyperbridge.xposed.mediacard.notification.layout.pixel.NotificationMediaPixelStyle
import com.d4viddf.hyperbridge.xposed.mediacard.notification.style.NotificationMediaCoverStyler
import com.d4viddf.hyperbridge.xposed.mediacard.notification.style.NotificationMediaForegroundStyler
...
object NotificationMediaCoverStyleHooker {
    ...
    fun hook(xposedModule: XposedModule, classLoader: ClassLoader) {
        ...
        NotificationMediaForegroundStyler.setAppliedListener { controller ->
            if (
                ...
                runCatching { refreshAppNameColor(controller) }
                    .onFailure { MediaCardLog.e(TAG, "刷新媒体卡片应用名称颜色失败", it) }
            }
        }
        ...
        if (handles.size != methods.size) {
            ...
            hookedClassLoaders.remove(classLoader)
            MediaCardLog.w(TAG, "通知中心媒体卡片 Hook 安装不完整")
        } else {
            MediaCardLog.d(TAG, "通知中心媒体卡片 Hook 已初始化: methods=${handles.size}")
        }
    }
        ...
        return when (method.declaringClass.name) {
            NotificationMediaHostClasses.VIEW_CONTROLLER -> when (method.name) {
                "onFullAodStateChanged" -> NotificationMediaFullAodHook(
                    ...
                    onFailure = { error ->
                        MediaCardLog.e(TAG, "应用通知中心 Full AOD 媒体样式失败", error)
                    }
                )
        ...
        return when (method.declaringClass.name) {
            NotificationMediaHostClasses.VIEW_CONTROLLER -> when (method.name) {
                "attach", "bindMediaData" ->
                    config.coverStyle != MediaCardConstants.NOTIFICATION_MEDIA_COVER_STYLE_DEFAULT ||
                            config.layoutStyle != MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_SYSTEM ||
                            config.hideTime ||
                            config.hideCustomActions
                "detach" ->
                    config.coverStyle ==
                            MediaCardConstants.NOTIFICATION_MEDIA_COVER_STYLE_ROTATING_CIRCLE ||
                            config.layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_COLOROS ||
                            config.layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_ONEUI ||
                            config.layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_MIUI ||
                            config.layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_PIXEL
                "setSeamless" ->
    ...
    private fun needsConstraintLayout(config: MediaCardRuntimeConfig.Notification): Boolean {
        return config.layoutStyle != MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_SYSTEM ||
                config.coverStyle == MediaCardConstants.NOTIFICATION_MEDIA_COVER_STYLE_HIDDEN ||
                config.hideCoverSource ||
                config.hideDeviceSwitch ||
                config.hideCustomActions ||
                config.hideTime ||
                config.actionAlignLeft ||
                config.actionOrder != MediaCardConstants.NOTIFICATION_MEDIA_ACTION_ORDER_DEFAULT
    }
    ...
    private class ControllerHook(private val methodName: String) : Hooker {
        override fun intercept(chain: Chain): Any? {
            ...
            if (methodName == "attach" || methodName == "bindMediaData") {
                ...
                }.onFailure { MediaCardLog.e(TAG, "应用通知中心媒体卡片样式失败", it) }
            }
            if (methodName == "setSeamless" &&
                ...
                runCatching { applyColorOsAppIcon(controller) }
                    .onFailure { MediaCardLog.e(TAG, "应用 ColorOS 媒体卡片应用图标失败", it) }
            }
            if (methodName == "setSeamless" &&
                runtimeConfig.notification.layoutStyle ==
                MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_ONEUI
                ...
                runCatching { applyOneUiStyle(controller) }
                    .onFailure { MediaCardLog.e(TAG, "应用 One UI 媒体卡片应用身份失败", it) }
            }
            if (methodName == "setSeamless" &&
                runtimeConfig.notification.layoutStyle ==
                MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_PIXEL
            ) {
                runCatching { refreshPixelAppIcon(controller) }
                    .onFailure { MediaCardLog.e(TAG, "刷新 Pixel 媒体卡片应用图标失败", it) }
            }
            if (methodName == "setSeamless" && shouldApplyUnifiedForeground(controller)) {
                runCatching { NotificationMediaAmbientFlowHooker.applyUnifiedForeground(controller) }
                    .onFailure { MediaCardLog.e(TAG, "刷新媒体卡片统一前景色失败", it) }
            }
            if (
                ...
                }.onFailure { MediaCardLog.e(TAG, "刷新媒体卡片应用身份失败", it) }
            }
            return result
        }
    }
    ...
    private class LayoutLoadHook : Hooker {
        override fun intercept(chain: Chain): Any? {
            ...
            }.onFailure { MediaCardLog.e(TAG, "应用通知中心媒体卡片约束失败", it) }
            return result
        }
    }
    ...
    private fun applyStyle(controller: Any, mediaData: Any?) {
        ...
        api.getSeekBar(holder)?.let { seekBar ->
            if (
                layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_IOS ||
                    layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_COLOROS ||
                    layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_ONEUI ||
                    layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_MIUI ||
                    layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_PIXEL
            ) {
            ...
            } else if (layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_MIUI) {
                applyMiuiMediaSeekBarPadding(seekBar)
            }
        }
        if (layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_COLOROS) {
            applyColorOsTimeAlignment(api, holder)
            applyColorOsAppIcon(api, controller, holder)
        } else if (layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_ONEUI) {
        ...
        } else if (layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_PIXEL) {
            NotificationMediaPixelStyle.apply(api, controller, holder, mediaData)
        }
        if (config.hideTime || layoutStyle == MediaCardConstants.NOTIFICATION_MEDIA_LAYOUT_STYLE_PIXEL) {
        ...
        if (
            ...
            MediaCardConstants.NOTIFICATION_MEDIA_COVER_STYLE_CIRCLE -> {
                MediaCoverRotationController.detach(albumImage)
        ...
        return runCatching { NotificationMediaHostApi.create(classLoader) }
            ...
            .onFailure {
                if (nativeUnavailableClassLoaders.add(classLoader)) {
                    MediaCardLog.w(TAG, "通知中心媒体卡片接口不可用: reason=${it.message}")
                }
            }
            .getOrNull()
    }
