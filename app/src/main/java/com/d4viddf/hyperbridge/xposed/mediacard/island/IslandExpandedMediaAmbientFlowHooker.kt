package com.d4viddf.hyperbridge.xposed.mediacard.island
...
import com.d4viddf.hyperbridge.xposed.mediacard.HookRuntimeRegistry
import com.d4viddf.hyperbridge.xposed.mediacard.compat.IslandAlbumCoverStyleHooker
import com.d4viddf.hyperbridge.xposed.mediacard.compat.IslandProbeUtils
import com.d4viddf.hyperbridge.xposed.mediacard.MediaAmbientFlowPalette
import com.d4viddf.hyperbridge.xposed.mediacard.MediaAmbientFlowPaletteExtractor
...
import com.d4viddf.hyperbridge.xposed.mediacard.background.MediaFlowArtwork
import com.d4viddf.hyperbridge.xposed.mediacard.background.MediaFlowBackgroundView
import com.d4viddf.hyperbridge.xposed.mediacard.background.MediaFlowOverlayLayout
import com.d4viddf.hyperbridge.xposed.mediacard.background.MediaFlowTimeline
import com.d4viddf.hyperbridge.xposed.mediacard.background.MediaFlowTone
import com.d4viddf.hyperbridge.xposed.mediacard.island.background.IslandExpandedBackgroundTarget
import com.d4viddf.hyperbridge.xposed.mediacard.island.background.IslandExpandedMediaBackgroundApi
import com.d4viddf.hyperbridge.xposed.mediacard.island.background.IslandExpandedMediaBackgroundController
import com.d4viddf.hyperbridge.xposed.mediacard.island.background.IslandExpandedMediaBackgroundHost
import com.d4viddf.hyperbridge.xposed.mediacard.island.background.IslandMediaBackgroundHostAdapter
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.coloros.IslandExpandedMediaColorOsAccessoryController
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.coloros.IslandExpandedMediaColorOsAccessoryViews
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.coloros.IslandExpandedMediaColorOsTimeController
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.oneui.IslandExpandedMediaOneUiAccessoryController
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.oneui.IslandExpandedMediaOneUiAccessoryViews
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.oneui.IslandExpandedMediaOneUiTimeController
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.oneui.IslandExpandedMediaOneUiActionController
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.miui.IslandExpandedMediaMiuiAppNameController
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.miui.IslandExpandedMediaMiuiActionController
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.miui.IslandExpandedMediaMiuiTimeController
import com.d4viddf.hyperbridge.xposed.mediacard.island.layout.pixel.IslandExpandedMediaPixelStyleController
import com.d4viddf.hyperbridge.xposed.mediacard.island.style.IslandExpandedMediaForegroundAccess
import com.d4viddf.hyperbridge.xposed.mediacard.island.style.IslandExpandedMediaForegroundStyler
...
import com.d4viddf.hyperbridge.xposed.mediacard.style.MediaCardForegroundPalette
import com.d4viddf.hyperbridge.xposed.mediacard.style.MediaCardForegroundPaletteResolver
...
object IslandExpandedMediaAmbientFlowHooker {
    ...
    private const val CUSTOM_FLOW_VIEW_TAG = "hyperbridge.media.island_expanded_media_custom_flow"
    ...
    private val colorExecutor = Executors.newSingleThreadExecutor { task ->
        Thread({
            ...
            task.run()
        }, "HyperBridge-Media-IslandMediaColor").apply { isDaemon = true }
    }
    ...
    fun hook(xposedModule: XposedModule, classLoader: ClassLoader) {
        ...
        if (installedHandles.size != api.hookMethods.size) {
            ...
            hookedClassLoaders.remove(classLoader)
            MediaCardLog.w(TAG, "展开态媒体流光 Hook 不完整，已移除全部 Hook")
        } else {
    ...
    fun refreshForUiMode() {
        ...
        binders.forEach { binder ->
            runCatching {
                ...
                MediaCardLog.w(TAG, "刷新超级岛媒体卡片主题失败", error)
            }
        }
    }
    ...
    fun resetMiniWindowBackgroundTransform() {
        ...
        synchronized(activeBinders) { activeBinders.toList() }.forEach { binder ->
            runCatching {
                ...
                MediaCardLog.e(TAG, "复位小窗返回时的媒体背景失败", error)
            }
        }
    }
    ...
    private class BinderHook(private val action: Action) : Hooker {
        override fun intercept(chain: Chain): Any? {
            ...
            if (
                ...
                hideDeviceSwitch() &&
                currentLayoutStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_COLOROS &&
                currentLayoutStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_ONEUI
            ) {
                return null
            }
            if (action == Action.DETACH) cleanupBinder(binder)
            ...
            runCatching {
                ...
                MediaCardLog.e(TAG, "应用展开态媒体流光模式失败", error)
            }
            return result
        }
    }
    ...
    private class PlaybackStartHook : Hooker {
        override fun intercept(chain: Chain): Any? {
            ...
            val suppressNativeFlow = if (IslandExpandedMediaBackgroundController.isActive()) {
                ...
                currentMode() == MediaCardConstants.ISLAND_EXPANDED_MEDIA_AMBIENT_FLOW_MODE_DISABLED ||
        ...
        override fun intercept(chain: Chain): Any? {
            ...
            if (IslandExpandedMediaBackgroundController.isActive()) {
                ...
                val foregroundApplied = runCatching {
                    ...
                    MediaCardLog.e(TAG, "保持展开态媒体前景色失败", error)
                    false
                }
                if (foregroundApplied) {
            ...
            return try {
                ...
                MediaCardLog.e(TAG, "应用原生浅色前景失败", error)
                IslandExpandedMediaForegroundStyler.restore(api, holder)
                val result = chain.proceed()
                syncMusicWave(binder, holder, api)
                syncLayoutAccessory(binder, holder, api)
    ...
    private fun installMiniBarTrackingHook(module: XposedModule, classLoader: ClassLoader) {
        runCatching {
            ...
            MediaCardLog.w(TAG, "跳过展开态媒体 MiniBar collector Hook: reason=${error.message}")
        }
    }
                ...
                player.post {
                    if (activeBinders.contains(binder) && shouldUseLightTheme(binder)) {
                        runCatching {
                            ...
                            MediaCardLog.e(TAG, "应用延后的实时通知背景失败", it)
                        }
                    }
                }
            }
    ...
    private fun shouldSuppressHeadGlowByPreference(): Boolean {
        ...
        val usesDefaultStyle =
            config.progressStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_PROGRESS_STYLE_DEFAULT
    ...
    private fun shouldUseLightTheme(context: Context): Boolean {
        return when (currentCardTheme()) {
            MediaCardConstants.MEDIA_CARD_THEME_ALWAYS_LIGHT -> true
            MediaCardConstants.MEDIA_CARD_THEME_ALWAYS_DARK -> false
    ...
    private fun applyMediaElements(binder: Any) {
        ...
        val isOneUi = layoutStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_ONEUI
        val isMiui = layoutStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_MIUI
        val isPixel = layoutStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_PIXEL
        ...
        val directCoverStyle = if (isOneUi || isMiui || isPixel) {
            ...
            coverStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_COVER_STYLE_HIDDEN
        ) {
        ...
        api.getHolders(binder).forEach { holder ->
            IslandExpandedMediaElementController.apply(
                ...
                keepAction4Slot =
                    (
                        layoutStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_IOS ||
                            layoutStyle ==
            ...
            syncMusicWave(binder, holder, api)
            if (layoutStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_COLOROS) {
    ...
    private fun syncColorOsAccessory(holder: Any, api: NativeApi) {
        if (currentLayoutStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_COLOROS) {
            return
        }
    ...
    private fun syncOneUiAccessory(binder: Any, holder: Any, api: NativeApi) {
        if (currentLayoutStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_ONEUI) {
            return
        }
    ...
    private fun syncMiuiAppName(binder: Any, holder: Any, api: NativeApi) {
        if (currentLayoutStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_MIUI) {
            return
        }
        val elements = api.getMediaElements(holder)
    ...
    private fun syncPixelStyle(binder: Any, holder: Any, api: NativeApi) {
        if (currentLayoutStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_PIXEL) {
            return
        }
        val context = api.getContext(binder)
    ...
    private fun syncMusicWave(binder: Any, holder: Any, api: NativeApi) {
        ...
        if (
            currentLayoutStyle() == MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_IOS
        ) {
    ...
    private fun applySeekBarTrackOffset(api: NativeApi, holder: Any) {
        if (
            ...
            currentLayoutStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_COLOROS &&
            currentLayoutStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_ONEUI &&
            currentLayoutStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_MIUI &&
            currentLayoutStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE_PIXEL
        ) {
            restoreSeekBarTrackOffset(api, holder)
            return
        }
    ...
    private fun applyFakeMediaElements(
        ...
        if (
            coverStyle == MediaCardConstants.ISLAND_EXPANDED_MEDIA_COVER_STYLE_DEFAULT &&
            !hideCoverSource &&
            !hideDeviceSwitch &&
            !hideCustomActions &&
            !hideTime &&
    ...
    private fun applyMode(binder: Any, allowCoverColor: Boolean) {
        ...
        when (currentMode()) {
            MediaCardConstants.ISLAND_EXPANDED_MEDIA_AMBIENT_FLOW_MODE_DISABLED -> {
                removeCustomFlow(binder)
                binderStates[binder]?.request?.incrementAndGet()
                views.forEach { view -> hideAmbientFlow(view, api) }
            }

            MediaCardConstants.ISLAND_EXPANDED_MEDIA_AMBIENT_FLOW_MODE_COVER_COLOR -> {
                removeCustomFlow(binder)
    ...
    private fun scheduleCustomFlowColors(
        ...
        runCatching {
            colorExecutor.execute {
                if (binderStates[binder] !== state || state.request.get() != request) return@execute
                val bitmap = runCatching { MediaArtworkSampler.sample(drawable) }.getOrNull()
                val artwork = bitmap?.let { sampled ->
                    try {
                        runCatching { MediaFlowArtwork.prepare(sampled) }
                            .onFailure { MediaCardLog.e(TAG, "提取展开态媒体柔光颜色失败", it) }
                            .getOrNull()
                    } finally {
                        sampled.recycle()
                    }
            ...
            MediaCardLog.e(TAG, "调度展开态媒体柔光取色失败", error)
        }
    }
    ...
    private fun scheduleCoverColors(binder: Any, primaryView: View, api: NativeApi) {
        ...
        runCatching {
            colorExecutor.execute {
                if (binderStates[binder] !== state || state.request.get() != request) return@execute
                val bitmap = runCatching { MediaArtworkSampler.sample(drawable) }.getOrNull()
                val palette = bitmap?.let { sampled ->
                    try {
                        runCatching {
                            MediaAmbientFlowPaletteExtractor.extractCoverMainColor(sampled)
                                ?.let(api::createPalette)
                        }
                            .onFailure { MediaCardLog.e(TAG, "提取展开态媒体颜色失败", it) }
                            .getOrNull()
                    } finally {
                        sampled.recycle()
                    }
            ...
            MediaCardLog.e(TAG, "调度展开态媒体取色任务失败", error)
        }
    }
    ...
    private fun currentMode(): Int {
        if (!MediaCardRuntimeConfig.current.enabled) {
            return MediaCardConstants.ISLAND_EXPANDED_MEDIA_AMBIENT_FLOW_MODE_DEFAULT
        }
        return MediaCardRuntimeConfig.current.islandExpanded.ambientFlowMode
    }

    private fun isCustomMode(mode: Int): Boolean =
        mode == MediaCardConstants.ISLAND_EXPANDED_MEDIA_AMBIENT_FLOW_MODE_CUSTOM_FULL

    private fun currentCardTheme(): Int {
        if (!MediaCardRuntimeConfig.current.enabled) {
            return MediaCardConstants.DEFAULT_HOOK_ISLAND_EXPANDED_MEDIA_CARD_THEME
        }
    ...
    private fun currentCoverStyle(): Int {
        if (!MediaCardRuntimeConfig.current.enabled) {
            return MediaCardConstants.ISLAND_EXPANDED_MEDIA_COVER_STYLE_DEFAULT
        }
    ...
    private fun currentLayoutStyle(): Int {
        if (!MediaCardRuntimeConfig.current.enabled) {
            return MediaCardConstants.DEFAULT_HOOK_ISLAND_EXPANDED_MEDIA_LAYOUT_STYLE
        }
        ...
        return runCatching { NativeApi.create(classLoader) }
            ...
            .onFailure {
                if (nativeUnavailableClassLoaders.add(classLoader)) {
                    MediaCardLog.w(TAG, "展开态媒体原生接口不可用: reason=${it.message}")
                }
            }
            .getOrNull()
    }
