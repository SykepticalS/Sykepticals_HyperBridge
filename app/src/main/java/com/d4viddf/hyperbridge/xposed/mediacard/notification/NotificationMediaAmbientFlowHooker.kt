...
object NotificationMediaAmbientFlowHooker {
    ...
    private const val VIEW_TAG = "hyperbridge.media.notification_media_ambient_flow"
    ...
    fun hook(xposedModule: XposedModule, classLoader: ClassLoader) {
        ...
        if (controllerClass == null) {
            ...
            MediaCardLog.w(TAG, "通知中心媒体控制器不可用")
            return
        }
        var installed = 0
        ...
        TARGET_METHOD_NAMES
            ...
            .forEach { method ->
                runCatching {
                    ...
                    MediaCardLog.e(TAG, "安装通知中心媒体 Hook 失败: method=${method.name}", error)
                }
            }
        ...
        if (!installedNativeUpdates.containsAll(NATIVE_BACKGROUND_UPDATE_METHODS)) {
            MediaCardLog.w(TAG, "通知中心原生背景接口不完整，跳过自定义背景 Hook")
        }
        ...
        runCatching {
            ...
            MediaCardLog.w(TAG, "通知中心进度条接口不可用: reason=${error.message}")
        }

        if (installed == 0) {
            hookedClassLoaders.remove(classLoader)
            MediaCardLog.w(TAG, "未找到兼容的通知中心媒体控制方法")
        } else {
            MediaCardLog.d(TAG, "通知中心媒体流光 Hook 已初始化: methods=$installed")
        }
    }
        ...
        override fun intercept(chain: Chain): Any? {
            ...
            if (action == Action.DETACH) {
                ...
                runCatching {
                    ...
                    MediaCardLog.e(TAG, "准备通知中心媒体卡片主题失败", it)
                }
            }
            if (action != Action.DETACH &&
    ...
    fun refreshForUiMode(nativeControllersToSkip: Set<Any> = emptySet()) {
        ...
        controllers.forEach { controller ->
            runCatching {
                ...
                MediaCardLog.w(TAG, "刷新通知中心媒体卡片主题失败", error)
            }
        }
    }
        ...
        return runCatching { CardThemeApi.create(classLoader) }
            ...
            .onFailure {
                if (themeUnavailableClassLoaders.add(classLoader)) {
                    MediaCardLog.w(TAG, "通知中心媒体主题接口不可用: reason=${it.message}")
                }
            }
            .getOrNull()
    }
    ...
    private fun syncView(controller: Any) {
        ...
        if (currentMode() != MediaCardConstants.NOTIFICATION_MEDIA_AMBIENT_FLOW_MODE_DISABLED) {
    ...
    private fun bind(controller: Any, mediaData: Any?) {
        ...
        if (mode == MediaCardConstants.NOTIFICATION_MEDIA_AMBIENT_FLOW_MODE_DISABLED) {
            removeView(controller)
            return
        }
        mediaData ?: return
        ...
        colorExecutor.execute {
            ...
            val palette = runCatching {
                ...
                MediaCardLog.e(TAG, "提取通知中心媒体封面颜色失败", error)
                null
            }
            view.post {
    ...
    private fun extractPalette(
        ...
        if (
            mode == MediaCardConstants.NOTIFICATION_MEDIA_AMBIENT_FLOW_MODE_DYNAMIC &&
            nativeApi != null
        ) {
    ...
    private fun hasVisibleCustomFlow(controller: Any): Boolean {
        if (currentMode() != MediaCardConstants.NOTIFICATION_MEDIA_AMBIENT_FLOW_MODE_CUSTOM_FULL) {
            return false
        }
    ...
    internal fun applyUnifiedForeground(controller: Any) {
        ...
        val mode = currentMode()
        if (mode != MediaCardConstants.NOTIFICATION_MEDIA_AMBIENT_FLOW_MODE_CUSTOM_FULL) {
            NotificationMediaForegroundStyler.clear(controller)
            return
        }
    ...
    private fun resolveNativeApi(classLoader: ClassLoader?): NativeMusicBgApi? {
        ...
        return runCatching { NativeMusicBgApi.create(classLoader) }
            ...
            .onFailure { error ->
                ...
                MediaCardLog.w(TAG, "原生 MusicBgView 不可用: reason=${error.message}")
            }
            .getOrNull()
    }
    ...
    private fun currentMode(): Int {
        if (!MediaCardRuntimeConfig.current.enabled) {
            return MediaCardConstants.NOTIFICATION_MEDIA_AMBIENT_FLOW_MODE_DISABLED
        }
        return MediaCardRuntimeConfig.current.notification.ambientFlowMode
    }

    private fun isCustomMode(mode: Int): Boolean =
        mode == MediaCardConstants.NOTIFICATION_MEDIA_AMBIENT_FLOW_MODE_CUSTOM_FULL
    ...
    private fun currentCardTheme(): Int {
        if (!MediaCardRuntimeConfig.current.enabled) {
            return MediaCardConstants.DEFAULT_HOOK_NOTIFICATION_MEDIA_CARD_THEME
        }
    ...
    private fun newColorExecutor() = Executors.newSingleThreadExecutor { task ->
        Thread({
            ...
            task.run()
        }, "HyperBridge-Media-MediaColor").apply { isDaemon = true }
    }
    ...
    private class CardThemeApi private constructor(
        ...
        fun apply(controller: Any, theme: Int) {
            ...
            val themedContext = when (theme) {
                MediaCardConstants.MEDIA_CARD_THEME_ALWAYS_LIGHT ->
            ...
            if (theme == MediaCardConstants.MEDIA_CARD_THEME_FOLLOW_SYSTEM) {
