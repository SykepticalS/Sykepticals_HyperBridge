package com.d4viddf.hyperbridge.xposed.mediacard.notification.switcher
...
import com.d4viddf.hyperbridge.xposed.mediacard.notification.NotificationMediaHostClasses
...
import com.d4viddf.hyperbridge.xposed.mediacard.style.MediaCardForegroundColors
...
internal object NotificationMediaSingleCardSwitcherHooker {
    ...
    fun hook(xposedModule: XposedModule, classLoader: ClassLoader) {
        ...
        if (layoutClass == null || viewControllerClass == null) {
            ...
            MediaCardLog.w(TAG, "跳过单卡片横滑 Hook: HyperOS 3 媒体类不可用")
            return
        }
        ...
        try {
            ...
            MediaCardLog.e(TAG, "单卡片横滑 Hook 安装失败，准备回滚", error)
        } finally {
            if (!installed) {
    ...
    private fun registerLayoutController(controller: Any, classLoader: ClassLoader) {
        ...
        if (mediaDataManager == null || viewController == null || sortUtils == null) {
            MediaCardLog.w(TAG, "跳过媒体列表注册: 原生字段缺失")
            return
        }
        ...
        if (bindMethod == null) {
            MediaCardLog.w(TAG, "跳过媒体列表注册: bindMediaData 不可用")
            return
        }
        ...
        runCatching {
            ...
            MediaCardLog.e(TAG, "注册 MediaData.Listener 失败", error)
        }
    }
    ...
    private fun rollbackHooks(module: XposedModule, handles: List<HookHandle>) {
        handles.asReversed().forEach { handle ->
            try {
                ...
                MediaCardLog.e(TAG, "回滚单卡片横滑 Hook 失败", error)
            }
        }
    }
    ...
    private fun ensureTouchHook(xposedModule: XposedModule, player: View) {
        val method = findTouchDispatchMethod(player.javaClass) ?: run {
            MediaCardLog.w(TAG, "跳过横滑触摸 Hook: player.dispatchTouchEvent 不可用")
            return
        }
        ...
        try {
            ...
            MediaCardLog.e(TAG, "安装媒体卡片触摸分发 Hook 失败", error)
        } finally {
    ...
    private class LayoutConstructorHook : Hooker {
        override fun intercept(chain: Chain): Any? {
            ...
            runCatching {
                ...
                MediaCardLog.e(TAG, "初始化媒体卡片选择器失败", error)
            }
            return result
        }
    }
    ...
    private class ControllerState(
        ...
        fun disableForFailure(reason: String, error: Throwable? = null) {
            ...
            MediaCardLog.w(TAG, "媒体卡片切换功能已停用：$reason", error)
        }
            ...
            when (event.actionMasked) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    runCatching {
                        ...
                        MediaCardLog.w(TAG, "取消原生媒体卡片触摸目标失败", error)
                    }
                    cancel.recycle()
        ...
        private fun bindSelected(data: Any) {
            ...
            try {
                ...
                MediaCardLog.e(TAG, "调用原生 bindMediaData 切换媒体失败", error)
        ...
        private fun onClearAllMediaCards() {
            runOnMain {
                ...
                if (clearMethod == null) {
                    MediaCardLog.w(TAG, "多媒体卡片批量清理入口不可用")
                    return@runOnMain
                }
                runCatching { clearMethod.invoke(mediaDataManager) }
                    .onFailure { error ->
                        MediaCardLog.w(TAG, "调用多媒体卡片批量清理失败", error)
                    }
            }
        }
