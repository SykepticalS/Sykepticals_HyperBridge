package com.d4viddf.hyperbridge.xposed.mediacard.island.background
...
import com.d4viddf.hyperbridge.xposed.mediacard.notification.background.MediaBackgroundRendererPool
import com.d4viddf.hyperbridge.xposed.mediacard.notification.background.NotificationMediaColorConfig
...
internal object IslandExpandedMediaBackgroundController {
    ...
    fun isActive(): Boolean {
        return currentStyle() != MediaCardConstants.ISLAND_EXPANDED_MEDIA_BACKGROUND_STYLE_DEFAULT
    }
    ...
    private fun renderTargets(
        ...
        executor.execute {
            ...
            val renderer = runCatching { MediaBackgroundRendererPool.get(classLoader) }
                .onFailure { error ->
                    MediaCardLog.e(TAG, "初始化展开态媒体背景渲染器失败", error)
                }
                .getOrNull()
            if (renderer == null) {
    ...
    private fun applyForeground(
        ...
        return try {
            ...
            MediaCardLog.e(TAG, "应用展开态媒体前景调色板失败", error)
            api.restoreCustomForeground(binder, holder)
            false
        }
    }
    ...
    private fun currentStyle(): Int {
        if (!MediaCardRuntimeConfig.current.enabled) {
            return MediaCardConstants.ISLAND_EXPANDED_MEDIA_BACKGROUND_STYLE_DEFAULT
        }
    ...
    private fun newExecutor(): ExecutorService = Executors.newSingleThreadExecutor { task ->
        Thread({
            ...
            task.run()
        }, "HyperBridge-Media-IslandMediaBackground").apply { isDaemon = true }
    }
