...
import com.d4viddf.hyperbridge.xposed.mediacard.notification.NotificationMediaDataIdentity
...
internal object NotificationMediaBackgroundController {
    ...
    fun isActive(controller: Any): Boolean {
        if (currentStyle() == MediaCardConstants.NOTIFICATION_MEDIA_BACKGROUND_STYLE_DEFAULT) return false
    ...
    private fun resolveRenderer(classLoader: ClassLoader?): NotificationMediaBackgroundRenderer? {
        ...
        return runCatching { MediaBackgroundRendererPool.get(classLoader) }
            .onFailure { error ->
                ...
                MediaCardLog.w(TAG, "通知中心 Monet 背景接口不可用: reason=${error.message}")
            }
            .getOrNull()
    }
    ...
    private fun currentStyle(): Int {
        if (!MediaCardRuntimeConfig.current.enabled) {
            return MediaCardConstants.NOTIFICATION_MEDIA_BACKGROUND_STYLE_DEFAULT
        }
    ...
    private fun newExecutor(): ExecutorService = Executors.newSingleThreadExecutor { task ->
        Thread({
            ...
            task.run()
        }, "HyperBridge-Media-MediaBackground").apply { isDaemon = true }
    }
