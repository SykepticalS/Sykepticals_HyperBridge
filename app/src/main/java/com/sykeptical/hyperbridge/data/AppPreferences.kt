...
import com.sykeptical.hyperbridge.data.db.AppSetting
import com.sykeptical.hyperbridge.data.db.SettingsKeys
import com.sykeptical.hyperbridge.models.CallStage
...
class AppPreferences(context: Context) {
    ...
    init {
        ...
        sharedScope.launch {
            migrationMutex.withLock {
                ...
                if (dao.getSetting(SettingsKeys.FLOATING_SETUP_PROMPT_SHOWN) == null) {
                    val hasSelectedApps = !dao.getSetting(SettingsKeys.ALLOWED_PACKAGES).isNullOrBlank()
                    // Established profiles have already passed their first activation. Do not
                    // surprise them with the new one-time dialog after an app update.
                    dao.insert(AppSetting(SettingsKeys.FLOATING_SETUP_PROMPT_SHOWN, hasSelectedApps.toString()))
                    dao.insert(AppSetting(SettingsKeys.FLOATING_SETUP_NOTICE_PENDING, "false"))
                } else if (dao.getSetting(SettingsKeys.FLOATING_SETUP_NOTICE_PENDING) == null) {
                    dao.insert(AppSetting(SettingsKeys.FLOATING_SETUP_NOTICE_PENDING, "false"))
                }
    ...
    suspend fun toggleApp(packageName: String, isEnabled: Boolean) {
        ...
        val shouldShowFirstActivationPrompt = FloatingNotificationSetup.shouldShowFirstActivationPrompt(
            ...
            isEnabled = isEnabled,
            promptAlreadyShown = dao.getSetting(SettingsKeys.FLOATING_SETUP_PROMPT_SHOWN).toBoolean(false)
        )
        ...
        if (shouldShowFirstActivationPrompt) {
            // Persist the lifetime marker when the event is created. The UI consumes the pending
            // trigger immediately on display, so process restarts cannot repeat the popup.
            save(SettingsKeys.FLOATING_SETUP_PROMPT_SHOWN, "true")
    ...
    private val GLOBAL_CALL_STAGES_KEY = "global_call_stages"
