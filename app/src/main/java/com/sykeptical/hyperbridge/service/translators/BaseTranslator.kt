...
import com.sykeptical.hyperbridge.models.BridgeAction
import com.sykeptical.hyperbridge.models.theme.ActionButtonMode
import com.sykeptical.hyperbridge.models.theme.ActionConfig
import com.sykeptical.hyperbridge.models.theme.HyperTheme
import com.sykeptical.hyperbridge.models.theme.ResourceType
import com.sykeptical.hyperbridge.models.theme.ThemeResource
import com.sykeptical.hyperbridge.service.visual.IconGeometry
import com.sykeptical.hyperbridge.service.visual.NotificationVisualSource
import com.sykeptical.hyperbridge.service.visual.ResolvedNotificationVisual
import com.sykeptical.hyperbridge.ui.screens.theme.getShapeFromId
...
abstract class BaseTranslator(
    ...
    /**
     * Removes transparent source margins and expands a glyph within a density-independent canvas.
     * SystemUI still controls the final action-button bounds, but receives artwork that uses the
     * available icon slot instead of a small, pre-padded bitmap.
     */
    protected fun createNormalizedActionGlyph(source: Bitmap, paddingPercent: Int): Bitmap {
        val safePadding = paddingPercent.coerceIn(0, 49)
        val cacheKey = "glyph:${safeGenerationId(source)}:${source.width}x${source.height}:$safePadding"
        processedActionIconCache.get(cacheKey)?.let { return it }

        val size = 96
        val output = createBitmap(size, size)
        val canvas = Canvas(output)
        val paddingPx = size * (safePadding / 100f)
        drawNormalizedBitmap(
            canvas,
            source,
            RectF(paddingPx, paddingPx, size - paddingPx, size - paddingPx),
            Paint(Paint.ANTI_ALIAS_FLAG or Paint.FILTER_BITMAP_FLAG)
        )
    ...
    private fun getVisibleBitmapBounds(bitmap: Bitmap): com.sykeptical.hyperbridge.service.visual.PixelBounds? {
